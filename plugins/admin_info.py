...
@Client.on_message(filters.command(["info", "stats"]) & filters.private & filters.user(ADMIN_ID),group=8872)
async def admin_info_hub(bot: Client, message: Message):
    ADMIN_STATES.pop(message.from_user.id, None)
    text = await build_info_text()
    await message.reply_text(text=text, reply_markup=get_admin_keyboard())

@Client.on_message(filters.command(["del", "delete"]) & filters.private & filters.user(ADMIN_ID),group=2551)
async def quick_delete_code(bot: Client, message: Message):
...
@Client.on_callback_query(filters.user(ADMIN_ID),group=6563)
async def admin_callbacks(bot: Client, cb: CallbackQuery):
    data = cb.data
...
@Client.on_message(filters.private & filters.text & filters.user(ADMIN_ID), group=8337)
async def admin_text_input(bot: Client, message: Message):
